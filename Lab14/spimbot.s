...
solve_puzzle:
    la      $t4, puzzle                                 # Load address of puzzle
    sw      $t4, REQUEST_PUZZLE                         # Store address of puzzle in Request_Puzzle to request for a puzzle

wait:
    lw      $t0, GET_BYTECOINS
    li      $t3, 1                                      # Set ANGLE_CONTROL to be absolute
    li      $t1, 45                                     # Set ANGLE to be 45
    sw      $t1, ANGLE
    sw      $t3, ANGLE_CONTROL
    bne     $t0, $0, start_movement
    j       wait
...
first_loop:
    beq     $t2, $t1, next_movement
    add     $t2, $t2, 1
    j       first_loop

next_movement:
    li      $t0, 0                                      # Set VELOCITY to 0
    sw      $t0, VELOCITY
    li      $t3, 1                                      # Set ANGLE_CONTROL to be absolute
    li      $t1, 0                                      # Set ANGLE to be 300
    sw      $t1, ANGLE
    sw      $t3, ANGLE_CONTROL
    sw      $0, SHOOT_UDP_PACKET
...
next_movement_2:
    li      $t3, 1                                      # Set ANGLE_CONTROL to be absolute
    li      $t1, 90                                      # Set ANGLE to be 900
    sw      $t1, ANGLE
    sw      $t3, ANGLE_CONTROL
    sw      $0, SHOOT_UDP_PACKET
    li      $t0, 10                                      # Set VELOCITY to 0
    sw      $t0, VELOCITY
    li      $t3, 1                                      # Set ANGLE_CONTROL to be absolute
    li      $t1, 60                                      # Set ANGLE to be 900
    sw      $t1, ANGLE
    sw      $t3, ANGLE_CONTROL
    li      $t1, 0
    li      $t2, 5000
...
loop:
    beq     $t1, $t2, shoot
    beq     $t1, $t3, change_angle
    add     $t1, $t1, 1
    j       loop

change_angle:
    li      $t3, 0                                     # Set ANGLE_CONTROL to be absolute
    li      $t6, 66                                    # Set ANGLE to be 300
    sw      $t6, ANGLE
    sw      $t3, ANGLE_CONTROL
    add     $t1, $t1, 1
    j       loop
...
bonk_interrupt:
    ...
    sw      $0, BONK_ACK
    li      $a0, 70
    sw      $a0, ANGLE              # Do a 70 degree rotation and move back
...
request_puzzle_interrupt:
    ...
    la      $a0, puzzle                                 # Load pointer for LightsOut
    la      $a1, solution                               # Load pointer for Solution
    li      $a2, 0                                      # Load 0 into row
    li      $a3, 0                                      # Load 0 into col
    jal     solve
    la      $t0, solution
    sw      $t0, SUBMIT_SOLUTION                         # Store pointer, solution, to SUBMIT_SOLUTION
...
toggle_light_row_less_if:
    ...
    mul     $t5, $t5, $t1
    add     $t5, $t5, $a1
    add     $t5, $t5, $t3  # assign  $t6 = (*::($t5) + $a3) % $t2
    lbu     $t6, 0($t5)
    add     $t6, $t6, $a3
    div     $t6, $t2
    mfhi    $t6            # assign  $t6 =>:: $t5
    sb      $t6, 0($t5)
